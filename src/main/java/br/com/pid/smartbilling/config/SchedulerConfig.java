...
import br.com.pid.smartbilling.model.SampleJob;
...
//    }
    ...
    @Bean
    public SchedulerFactoryBean schedulerFactoryBean(DataSource dataSource, JobFactory jobFactory,
                                                     @Qualifier("cronJobTrigger") Trigger sampleJobTrigger) throws IOException {
        SchedulerFactoryBean factory = new SchedulerFactoryBean();
        //this allows to update triggers in DB when updating settings in config file:
        factory.setOverwriteExistingJobs(true);
        factory.setDataSource(dataSource);
...
//    }
    ...
    public CronTriggerFactoryBean cronJobTrigger(@Qualifier("sampleJobDetail") JobDetail jobDetail,
                                                     @Value("${cron.frequency.jobwithcrontrigger}") String cronExpression) {
